...
| [`paik-sync-jira`](paik-sync-jira/SKILL.md) | Refresh ticket-derived status using the Atlassian MCP server. |
| [`paik-sync-confluence`](paik-sync-confluence/SKILL.md) | Verify/refresh knowledge-base links. |
| [`paik-sync-api-spec`](paik-sync-api-spec/SKILL.md) | Pull current API spec version/metadata. |
