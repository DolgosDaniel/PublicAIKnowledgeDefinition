...
Six common mistakes, each with a minimal broken snippet and the exact error
...
`tools/test_paik_validate.py` for the actual test cases these come from.
...
environment can't share an id either. Rename one of them.
...
```
ERROR: project.md: components entry 'environments/dev.md' points at a 'environment' document, expected 'component'
```

The path resolved to a real file, but that file's own `kind` doesn't match what a `components`
...
A `components/b.md` file exists on disk with `kind: component`, but `project.md`'s `components`
array never lists it:
...
section 3). If you don't know what kind to use yet, that's what `paik-spec/LINKS.md` is for.
...
only ever cross-reference each other (SPEC.md section 4) — if you need to point at something
